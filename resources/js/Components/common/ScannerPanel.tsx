  ...
  if (!scanning) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <div className="w-24 h-24 rounded-2xl bg-role/5 flex items-center justify-center">
          <QrCode size={48} className="text-role" />
        </div>
        <p className="text-gray-500 text-sm">{idleHint}</p>
        <button
          onClick={onStart}
          className="px-8 py-3 bg-role text-white rounded-xl font-semibold hover:bg-role-dark transition-all"
        >
          Start Scanner
        </button>
      </div>

    ...
    return (
        <div className="relative flex items-center gap-1">
            ...
            <div ref={scrollRef} className="flex overflow-x-auto space-x-2 scrollbar-hide flex-1 min-w-0" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {classes.map((cls) => (
                    <button key={cls.classroom_id} onClick={() => onSelect(cls.classroom_id)} className={`flex-shrink-0 px-6 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${activeClassId === cls.classroom_id ? 'bg-[#1c3068] text-white shadow-md' : 'text-gray-500 hover:text-[#1c3068] hover:bg-gray-50'}`}>{cls.name}</button>
                ))}
            </div>

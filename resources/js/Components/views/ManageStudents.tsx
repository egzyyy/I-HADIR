    ...
    return (
        <div className="relative flex items-center gap-1">
            ...
            <div ref={scrollRef} className="flex overflow-x-auto space-x-2 scrollbar-hide flex-1 min-w-0" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                {classes.map((cls) => (
                    <button key={cls.classroom_id} onClick={() => onSelect(cls.classroom_id)} className={`flex-shrink-0 px-6 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${activeClassId === cls.classroom_id ? 'bg-role text-white shadow-md' : 'text-gray-500 hover:text-role hover:bg-gray-50'}`}>{cls.name}</button>
                ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full min-w-0 flex flex-col space-y-6 overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-role uppercase tracking-wide">Manage {moduleName} Students</h2>
                ...
                <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end pr-4 border-r border-gray-200 hidden sm:flex">
                        ...
                        <span className={`text-xl font-black ${maxCapacity !== null && currentProjectedTotal >= maxCapacity ? 'text-red-500' : 'text-role'}`}>
                    ...
                    <button onClick={handleSave} disabled={saving || loadingStudents} className="bg-role hover:bg-role-dark text-white px-6 py-2.5 rounded-lg font-bold shadow-md transition-all disabled:opacity-50 whitespace-nowrap">
                        Save Changes
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden w-full min-w-0">
                <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-role">Student Roster</h3>
                            <p className="text-xs text-gray-400 mt-1">Total {filteredStudents.length} students in class</p>
                        </div>
                    </div>
                    <div className="relative w-full sm:w-72">
                        <Search size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
                        <input type="text" placeholder="Search by name or IC..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:border-role focus:bg-white outline-none transition-all" />
                    </div>
                </div>
                            ...
                            <tr className="bg-gray-50 border-b border-gray-200 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                ...
                                <th className="px-6 py-4 w-16 bg-gray-50">
                                    <label className="flex items-center justify-center cursor-pointer">
                                        ...
                                        <div className={`w-5 h-5 rounded flex items-center justify-center transition-all ${availableCount === 0 ? 'bg-gray-200 border-gray-200 cursor-not-allowed' : isAllSelected ? 'bg-role border-role' : 'border-2 border-gray-300 hover:border-role'}`}>
                                ...
                                filteredStudents.map((student, index) => {
                                    ...
                                    return (
                                        <tr key={student.student_id} className={`transition-colors ${student.is_disabled ? 'bg-gray-50/50' : 'hover:bg-blue-50/30'}`} onClick={() => handleToggle(student.student_id, student.is_disabled)}>
                                            ...
                                            <td className={`px-6 py-4 text-sm font-bold ${student.is_disabled ? 'text-gray-400' : 'text-role'}`}>{student.name}</td>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <label className={`flex items-center justify-center ${student.is_disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <div className={`w-5 h-5 rounded flex items-center justify-center transition-all ${student.is_disabled ? 'bg-gray-200 border-gray-200' : isChecked ? 'bg-role border-role' : 'border-2 border-gray-300'}`}>

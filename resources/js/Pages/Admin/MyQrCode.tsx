...
const MyQrCodeContent = () => {
  ...
  return (
    ...
    >
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-[#1c3068]">My QR Code</h2>
        ...
        <div className="p-8 flex flex-col items-center gap-6">
          {loading && (
            <div className="w-[300px] h-[300px] flex items-center justify-center">
              <div className="w-12 h-12 border-4 border-[#1c3068] border-t-transparent rounded-full animate-spin" />
            </div>
          )}
          ...
          {!loading && !error && data && (
            <>
              <div className="border-2 border-[#1c3068]/10 rounded-2xl p-6 bg-gradient-to-br from-white to-gray-50/50 shadow-inner">
                <canvas ref={canvasRef} />
              </div>

              <div className="text-center space-y-1">
                <p className="text-lg font-black text-[#1c3068]">{data.name}</p>
                <p className="text-sm font-bold text-[#c53336]">{data.label}</p>
              </div>

              <button
                onClick={handleDownload}
                className="flex items-center justify-center gap-2 px-8 py-3 bg-[#1c3068] text-white rounded-xl font-bold hover:bg-[#152450] transition-all text-sm shadow-lg shadow-blue-900/20 transform hover:-translate-y-0.5"
              >
              ...
              <div className="w-full mt-2 p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                <div className="flex items-start gap-3">
                  <QrCode size={20} className="text-[#1c3068] mt-0.5 flex-shrink-0" />
